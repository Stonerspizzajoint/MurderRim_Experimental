...
namespace WorkerDronesMod
{
    ...
    public static class GeneHediffSwitcherUtility
    {
        ...
        public static PawnState GetPawnState(Pawn pawn)
        {
            ...
            // 1) HighHeat (as determined by Gene_BasicSolver)
            if (pawn.genes != null)
            {
                var basicSolver = pawn.genes.GetFirstGeneOfType<Gene_BasicSolver>();
                if (basicSolver != null && basicSolver.InitialResourceMax > 0f &&
                    basicSolver.Heat / basicSolver.InitialResourceMax > 0.6f)
                {
