...
namespace WorkerDronesMod
{
    public static class RoboticLimbRegenerator
    {
        ...
        /// </summary>
        public static void ProcessRegeneration(Pawn pawn, RegenOptions regenOptions)
        {
            ...
            {
                ProcessRegenerationForPart(pawn, part, regenOptions);
            }
        }
        ...
        /// </summary>
        public static float CalculateHeatForPawn(Pawn pawn, RegenOptions regenOptions)
        {
            float totalHeat = 0f;
            foreach (var hediff in pawn.health.hediffSet.hediffs)
            {
                ...
                {
                    totalHeat += CalculatePartHeat(hediff.Part.coverage, hediff.Severity, regenOptions);
                }
            }
            return totalHeat;
        }
        ...
        /// </summary>
        private static float CalculatePartHeat(float coverage, float severity, RegenOptions regenOptions)
        {
            return regenOptions.baseHeatPerCoverage * coverage * (1 - (severity * regenOptions.severityHeatFactor));
        }

        /// <summary>
        /// Processes regeneration for a single body part.
        /// </summary>
        public static void ProcessRegenerationForPart(Pawn pawn, BodyPartRecord part, RegenOptions regenOptions)
        {
            ...
            // If the part is missing, check for warmup before restoring.
            if (pawn.health.hediffSet.PartIsMissing(part))
            {
                // Track when the part went missing
                if (!partMissingTick.ContainsKey(key))
                    partMissingTick[key] = currentTick;

                // Is any other part being regenerated?
                bool otherPartRegenerating = pawn.health.hediffSet.hediffs
                    .Any(h => h.def == ReconstructionDef && h.Part != part && h.Severity < 1f);

                int requiredDelay = otherPartRegenerating
                    ? regenOptions.additionalDamageLimbDelayTicks
                    : regenOptions.missingLimbWarmupTicks;

                int ticksSinceMissing = currentTick - partMissingTick[key];
                if (ticksSinceMissing < requiredDelay)
                {
                    if (DebugSettings.godMode)
                        Log.Message($"[RoboticLimbRegenerator] {part.def.defName} missing for only {ticksSinceMissing} ticks, waiting for {requiredDelay}.");
                    return;
                }
                ...
                // Clean up tracking after restoration
                partMissingTick.Remove(key);
            }
            else
            {
                if (DebugSettings.godMode)
        ...
        private static void FinalizeReconstruction(Pawn pawn, BodyPartRecord part, Hediff reconstructionHediff)
        {
            ...
            // If the head was regenerated, refresh pawn graphics so the head is visible.
            if (part.def == BodyPartDefOf.Head && pawn.Drawer != null && pawn.Drawer.renderer != null)
            {
                pawn.Drawer.renderer.SetAllGraphicsDirty();
                if (DebugSettings.godMode)
                    Log.Message($"[RoboticLimbRegenerator] Marked all graphics dirty for {pawn.LabelShort} after head regeneration.");
            }
