...
namespace WorkerDronesMod
{
    public class ThinkNode_ConditionalHeatAboveThreshold : ThinkNode_Conditional
    {
        ...
        protected override bool Satisfied(Pawn pawn)
        {
            ...
            // Retrieve the pawn's basic solver gene.
            Gene_BasicSolver heatGene = pawn.genes?.GetFirstGeneOfType<Gene_BasicSolver>();
            if (heatGene == null || heatGene.InitialResourceMax <= 0f)
                return false;

            // Calculate the current heat as a fraction of the maximum.
            float heatPercent = heatGene.Heat / heatGene.InitialResourceMax;
