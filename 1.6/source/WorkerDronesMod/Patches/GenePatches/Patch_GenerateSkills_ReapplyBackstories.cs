...
namespace WorkerDronesMod.Patches
{
    ...
    public static class Patch_GenerateSkills_ReapplyBackstories
    {
        static void Postfix(Pawn pawn, PawnGenerationRequest request)
        {
            ...
            try
            {
                // 1) Gather allowed spawnCategories from PawnKindDef if present, else from gene extension
                List<string> allowedCats = null;

                // Try to get from PawnKindDef
                if (pawn.kindDef is PawnKindDef kindDef && kindDef.backstoryCategories != null && kindDef.backstoryCategories.Any())
                {
                    allowedCats = kindDef.backstoryCategories.ToList();
                }
                else
                {
                    // Fallback to gene extension
                    allowedCats = pawn.genes.GenesListForReading
                ...
                if (allowedCats == null || !allowedCats.Any()) return;
