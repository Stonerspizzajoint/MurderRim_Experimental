...
namespace WorkerDronesMod
{
    public static class SolverRegenerationUtil
    {
        ...
        private static void HandleWoundHealing(Pawn pawn, Gene_BasicSolver gene, List<Hediff> hediffs)
        {
            ...
            float healingFactor = IsACoreHeart(pawn) ? 0.25f : 1f;
            ...
            if (effectiveRegen > 0.0001f)
            {
                ...
                float ambientMultiplier = HeatUtil.HeatAmbientMultiplier(pawn.AmbientTemperature);
        ...
        // Checks if a missing part is old enough for regeneration (simple version)
        private static bool IsMissingPartReadyForRegeneration(Hediff_MissingPart missingPart, int initialDelay)
        {
            return missingPart != null && missingPart.ageTicks >= initialDelay;
        }
        ...
        private static bool IsACoreHeart(Pawn pawn)
        {
            return false;
        }
    }
